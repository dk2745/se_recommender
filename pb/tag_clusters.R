...
membership(wc)
plot(wc, graph)
plot(graph, vertex.color=membership(wc), edge.curved=FALSE)

#=========================================================================================
#Testing the significance of a community
wc <- walktrap.community(graph)
community.significance.test <- function(graph, t1) {
  subgraph <- induced.subgraph(graph, t1)
  in.degrees <- degree(subgraph)
  out.degrees <- degree(graph, t1) - in.degrees
  wilcox.test(in.degrees, out.degrees)
}
community.significance.test(graph,t1)
